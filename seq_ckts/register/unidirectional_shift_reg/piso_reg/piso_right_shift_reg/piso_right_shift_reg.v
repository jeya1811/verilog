...
module piso_right_shift_reg #(parameter Width= 2)(
  input clk, rst, load,
  input [Width-1:0] in,
  output out
);
...
genvar i;
generate
  for(i= 0; i< Width; i= i+ 1) begin: piso_right
    if(i== Width-1)
...
module tb_piso_right_shift_reg;
localparam Width= 4;
reg clk= 1'b0;
reg rst= 1'b1;
reg load;
...
piso_right_shift_reg #(.Width(Width)) dut(.clk(clk), .rst(rst), .load(load), .in(in), .out(out));
...
initial begin
  $dumpfile(".vcd");
  $dumpvars(0, tb_piso_right_shift_reg);
