...
module sipo_left_shift_reg #(parameter Width= 2)(
  input clk, rst, in,
  output [Width-1:0] out
);
genvar i;
generate
  for(i= 0; i< Width; i= i+ 1) begin: sipo_left
    if(i== 0)
...
sipo_left_shift_reg #(.Width(Width)) dut(.clk(clk), .rst(rst), .in(in), .out(out));
...
initial begin
  $dumpfile(".vcd");
  $dumpvars(0, tb_sipo_left_shift_reg);
